...
export default function RealTimeData() {
  ...
  const sendApprovalToFirebase = (data) => {
    const newApprovalRef = ref(database, 'boltonApprovals');
  ...
  const sendNotApprovalToFirebase = (matchingPaymentsystem) => {
    ...
    if (matchingPaymentsystem) {
      const paymentRef = ref(database, `BoltonPaymentsystem/${matchingPaymentsystem.key}`);
  ...
  useEffect(() => {
    const scehueleRotaRef = ref(database, 'BoltonScehueleRota');
    const paymentsystemRef = ref(database, 'BoltonPaymentsystem');
