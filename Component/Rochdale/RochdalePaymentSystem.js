...
import RochdaleSiteHeader from '../Rochdale/RochdaleSiteHeader'
...
export default function RealTimeData() {
  ...
  const sendApprovalToFirebase = (data) => {
    const newApprovalRef = ref(database, 'RochdaleApprovals');
  ...
  const sendNotApprovalToFirebase = (matchingPaymentsystem) => {
    ...
    if (matchingPaymentsystem) {
      const paymentRef = ref(database, `RochdalePaymentsystem/${matchingPaymentsystem.key}`);
  ...
  useEffect(() => {
    const scehueleRotaRef = ref(database, 'RochdaleScehueleRota');
    const paymentsystemRef = ref(database, 'RochdalePaymentsystem');
