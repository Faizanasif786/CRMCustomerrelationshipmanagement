...
import LaylanSiteHeader from '../Layland/LaylanSiteHeader'
...
export default function RealTimeData() {
  ...
  const sendApprovalToFirebase = (data) => {
    const newApprovalRef = ref(database, 'LaylandApprovals');
  ...
  const sendNotApprovalToFirebase = (matchingPaymentsystem) => {
    ...
    if (matchingPaymentsystem) {
      const paymentRef = ref(database, `LaylandPaymentsystem/${matchingPaymentsystem.key}`);
  ...
  useEffect(() => {
    const scehueleRotaRef = ref(database, 'LaylandScehueleRota');
    const paymentsystemRef = ref(database, 'LaylandPaymentsystem');
