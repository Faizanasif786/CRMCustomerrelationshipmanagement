...
import MachesterSiteHeader from '../Manchester/MachesterSiteHeader'
...
export default function RealTimeData() {
  ...
  const sendApprovalToFirebase = (data) => {
    const newApprovalRef = ref(database, 'MachesterApprovals');
  ...
  const sendNotApprovalToFirebase = (matchingPaymentsystem) => {
    ...
    if (matchingPaymentsystem) {
      const paymentRef = ref(database, `MachesterPaymentsystem/${matchingPaymentsystem.key}`);
  ...
  useEffect(() => {
    const scehueleRotaRef = ref(database, 'MachesterScehueleRota');
    const paymentsystemRef = ref(database, 'MachesterPaymentsystem');
